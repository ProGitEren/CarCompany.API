...
public class RegisterAdminQueryHandler : IQueryHandler<RegisterAdminQuery, UserDto>
{
    ...
    public RegisterAdminQueryHandler(EncryptionService encryptionService, UserManager<AppUsers> userManager,
    ...
    public async Task<Result<UserDto>> Handle(RegisterAdminQuery request, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            _logger.Information("Using provided encrypted password for user {Email}, CorrelationId: {CorrelationId}", request.Register.Email, correlationId);
        }
        catch
        {
            // If decryption fails, assume the password is not encrypted and encrypt it
            password = request.Register.EncryptedPassword; // if the API administrator enters a normal password 
            _logger.Information("Encrypting provided password for user {Email}, CorrelationId: {CorrelationId}", request.Register.Email, correlationId);
        }
        ...
        {
            _logger.Error(ex, "Failed to add address for user {Email}, CorrelationId: {CorrelationId}", request.Register.Email, correlationId);
